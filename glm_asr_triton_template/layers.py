...
class Linear:
    ...
    def _forward_torch(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.weight.device != x.device:
            self.weight = self.weight.to(x.device)
            self._weight_fp16_t = None
        if self._weight_fp16_t is None:
            self._weight_fp16_t = self.weight.to(torch.float16).t().contiguous()
        output = (x_2d @ self._weight_fp16_t).to(torch.float32)
        # if self.weight.device != x.device:
        #     self.weight = self.weight.to(x.device)
        # output = (x_2d @ self.weight.to(torch.float16).t()).to(torch.float32) #output = x_2d @ self.weight.t()
