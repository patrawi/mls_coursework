...
def apply_rope_kernel(
    ...
    mask = offs < half_dim
    x1 = tl.load(x_ptr + pid_bh * stride_x0 + pid_s * stride_x1 + offs * stride_x2, mask=mask, other=0.0).to(tl.float32)
    x2 = tl.load(x_ptr + pid_bh * stride_x0 + pid_s * stride_x1 + (offs + half_dim) * stride_x2, mask=mask, other=0.0).to(tl.float32)
    cos = tl.load(cos_ptr + pid_s * stride_cos0 + offs * stride_cos1, mask=mask, other=0.0).to(tl.float32)
    sin = tl.load(sin_ptr + pid_s * stride_cos0 + offs * stride_cos1, mask=mask, other=0.0).to(tl.float32)
    x1_new = x1 * cos - x2 * sin
    x2_new = x2 * cos + x1 * sin
    tl.store(out_ptr + pid_bh * stride_o0 + pid_s * stride_o1 + offs * stride_o2, x1_new, mask=mask)
    tl.store(out_ptr + pid_bh * stride_o0 + pid_s * stride_o1 + (offs + half_dim) * stride_o2, x2_new, mask=mask)


@triton.jit
def copy_passthrough_kernel(
    x_ptr, out_ptr, n_elements, start_offset,
    stride_x0, stride_x1, stride_x2,
    stride_o0, stride_o1, stride_o2,
    BLOCK: tl.constexpr,
):
    ...
    x = tl.load(x_ptr + pid_bh * stride_x0 + pid_s * stride_x1 + (start_offset + offs) * stride_x2, mask=mask, other=0.0)
    tl.store(out_ptr + pid_bh * stride_o0 + pid_s * stride_o1 + (start_offset + offs) * stride_o2, x, mask=mask)
...
def _apply_rope_fused(x, cos, sin, half_dim, head_dim):
    batch, num_heads, seq_len, _ = x.shape
    cos = cos[:seq_len]
    sin = sin[:seq_len]
    cos_half = cos[:, :half_dim].contiguous()
    sin_half = sin[:, :half_dim].contiguous()
    x_flat = x.reshape(batch * num_heads, seq_len, head_dim).contiguous()
    out_flat = torch.empty_like(x_flat)
    BLOCK_HD = next_power_of_two(half_dim)
    ...
    passthrough = head_dim - 2 * half_dim
    if passthrough > 0:
        BLOCK_PASS = next_power_of_two(passthrough)
        copy_passthrough_kernel[grid](
            x_flat, out_flat, passthrough, 2 * half_dim,
            x_flat.stride(0), x_flat.stride(1), x_flat.stride(2),
            out_flat.stride(0), out_flat.stride(1), out_flat.stride(2),
            BLOCK=BLOCK_PASS,
        )
...
def apply_rotary_pos_emb(
    ...
    if q.is_cuda:
        q_out = _apply_rope_fused(q, cos, sin, half_dim, head_dim)
        k_out = _apply_rope_fused(k, cos, sin, half_dim, head_dim)
    else:
